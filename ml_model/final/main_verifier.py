...
def run_all_verifications(title: str, content: str, url: str) -> dict:
    ...
    # Run source credibility check
    source_result = get_source_credibility(domain)
    source_score = source_result.get("score", 0) or 0
    source_note = "This source is generally considered reliable." if source_score >= 70 else "This source may require caution depending on the topic."
    ...
    final_verdict, explanation = decide_final_verdict(pattern_result, source_result, factual_result)
...
def decide_final_verdict(pattern: dict, source: dict, factual: dict) -> tuple:
    ...
    source_score = source.get("score", 0) or 0
